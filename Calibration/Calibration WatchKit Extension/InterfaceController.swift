...
//                if (data.gravity.x > 0.85 && data.gravity.y < 0.1 && data.gravity.z < 0.1 && data.attitude.pitch > -3.0 && data.attitude.pitch < 3.0 && data.attitude.roll > 87.0 && data.attitude.roll < 93.0) {
//                    print("Calibrated")
...
//                let gravityDifferenceX : Bool = self!.gravityOffsetValue(currentValue: data.userAcceleration.x, referenceValue: data.gravity.x)
//                let gravityDifferenceY : Bool = self!.gravityOffsetValue(currentValue: data.userAcceleration.y, referenceValue: data.gravity.y)
//                let gravityDifferenceZ : Bool = self!.gravityOffsetValue(currentValue: data.userAcceleration.z, referenceValue: data.gravity.z)
            }
        }
    }
...
//
//        filterX = KalmanFilter(stateEstimatePrior: 0.0, errorCovariancePrior: 1)
//        filterY = KalmanFilter(stateEstimatePrior: 0.0, errorCovariancePrior: 1)
//        filterZ = KalmanFilter(stateEstimatePrior: 0.0, errorCovariancePrior: 1)
    }
    
    func gravityOffsetValue(currentValue : Double , referenceValue: Double) -> Bool {
    ...
    func gravityOffsetDirection(currentDirection : Double , referenceDirection: Double) -> Bool {
        if (currentDirection == 0 && referenceDirection == 0) {
            return true
        } else if (currentDirection < 0 && referenceDirection < 0) {
            return true
        } else if (currentDirection > 0 && referenceDirection > 0) {
            return true
        } else {
